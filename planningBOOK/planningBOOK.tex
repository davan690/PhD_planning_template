...
\mainmatter
\hypertarget{phd-planning}{%
\chapter{PhD planning}\label{phd-planning}}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# install.packages("tinytex")}
\NormalTok{tinytex}\OperatorTok{::}\KeywordTok{parse_packages}\NormalTok{(}\StringTok{"./planningBOOK.log"}\NormalTok{)}
\CommentTok{# tinytex::install_tinytex()}
\CommentTok{# tinytex:::install_yihui_pkgs()}
\NormalTok{tinytex}\OperatorTok{:::}\KeywordTok{is_tinytex}\NormalTok{()}

\CommentTok{# tinytex:::check_tl_version(x = "preamble.tex")}
\NormalTok{tinytex}\OperatorTok{::}\KeywordTok{parse_install}\NormalTok{(}\StringTok{"./planningBOOK.log"}\NormalTok{)}
...
\CommentTok{# Here is a complete list about the "missing" packages:}

\CommentTok{# fontenc: part of latex distribution}
\CommentTok{# lmodern: is called "lm"}
\CommentTok{# graphicx: part of the "graphics" package (which is part of "required")}
\CommentTok{# lscape: part of the "graphics" package}
\CommentTok{# otf: could be japanese-otf, font-otf, or japanese-otf-uptex or any combination thereof}
\CommentTok{# plext: part of platex, so maybe also of uplatex}
\CommentTok{# verbatim: part of the "required" package}
\CommentTok{# devtools::install_github("njtierney/chaletex")}
\KeywordTok{library}\NormalTok{(chaletex)}
\NormalTok{lat_packages_needed <-}\StringTok{ }\KeywordTok{read_tex}\NormalTok{(}\StringTok{"./PhD_template.tex"}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{tex_extract_preamble}\NormalTok{() }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{tex_extract_pkgs}\NormalTok{() }

\CommentTok{# tinytex::tlmgr()}


\NormalTok{lat_packages_needed }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{tex_install_pkgs}\NormalTok{()}
...
\textbf{bookdown} package (Xie \protect\hyperlink{ref-R-bookdown}{2020})
